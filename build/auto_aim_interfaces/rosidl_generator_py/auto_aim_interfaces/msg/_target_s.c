...
bool auto_aim_interfaces__msg__target__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // d_radius
    PyObject * field = PyObject_GetAttrString(_pymsg, "d_radius");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->d_radius = PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
  {  // dz
...
PyObject * auto_aim_interfaces__msg__target__convert_to_py(void * raw_ros_message)
{
  ...
  {  // d_radius
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->d_radius);
    {
      int rc = PyObject_SetAttrString(_pymessage, "d_radius", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
