#ifndef BUFF_DETECTOR_THREAD_POOL_HPP
#define BUFF_DETECTOR_THREAD_POOL_HPP
...
inline std::vector<std::unique_ptr<YOLO>> create_yolov8s(
  const std::string & model_path, int number, double conf_threshold, const std::string & device)
{
  std::vector<std::unique_ptr<YOLO>> yolov8s;
  for (int i = 0; i < number; i++) {
    // 使用make_unique创建实例
    auto yolo_instance = std::make_unique<YOLO>(model_path, conf_threshold, device);    
    yolov8s.push_back(std::move(yolo_instance));
  }
  return yolov8s;
}
...
// }
  ...
  Frame dequeue()
  {
    ...
    Frame item = std::move(main_queue_.front());
    main_queue_.pop();
    return item;
  }


  bool dequeue_timeout(Frame & item, std::chrono::milliseconds timeout = std::chrono::milliseconds(100))
  {
    std::unique_lock<std::mutex> lock(mutex_);
