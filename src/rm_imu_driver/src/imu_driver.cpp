...
namespace rm_imu_driver
{
  RM_imu_driver::RM_imu_driver(const rclcpp::NodeOptions &options) : Node("imu_driver", options)
  {
    declare_params();
    read_params();
    ...
    rec_thread_ = std::thread(&RM_imu_driver::get_imu_data_thread, this);

    RCLCPP_INFO(this->get_logger(),"IMU_Driver initialized !");
  }

  RM_imu_driver::~RM_imu_driver()
  {
    stop_thread_ = true;
  ...
  void RM_imu_driver::init_serial()
  {
    try
    {
      ...
      RCLCPP_INFO(this->get_logger(), "[RM_imu_driver] serial port opened");
    }
  ...
  void RM_imu_driver::get_imu_data_thread(){
    while (!stop_thread_)
    {
      if (!serial_.isOpen())
      {
  ...
  void RM_imu_driver::declare_params(){
    try {
  ...
  void RM_imu_driver::read_params(){
...
RCLCPP_COMPONENTS_REGISTER_NODE(rm_imu_driver::RM_imu_driver)