...
void RMPlannerNode::publishMarkers(const auto_aim_interfaces::msg::Target &target_msg,
  ...
  if (target_msg.tracking) {
    ...
    selection_marker_.pose.position.y = distance * sin(gimbal_cmd.target_yaw * M_PI / 180);
    selection_marker_.pose.position.x = distance * cos(gimbal_cmd.target_yaw * M_PI / 180);
    selection_marker_.pose.position.z = distance * sin(gimbal_cmd.target_pitch * M_PI / 180);
